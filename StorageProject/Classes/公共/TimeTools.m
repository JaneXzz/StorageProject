...
+(void)initialize{
    ...
    dispatch_once(&onceToken, ^{
        ...
        chineseYear = @[@"甲子鼠年", @"乙丑牛年", @"丙寅虎年", @"丁卯兔年", @"戊辰龙年", @"己巳蛇年", @"庚午马年",
                        @"辛未羊年", @"壬申猴年", @"癸酉鸡年", @"甲戌狗年", @"乙亥猪年",
                        
                        @"丙子鼠年", @"丁丑牛年", @"戊寅虎年", @"己卯兔年", @"庚辰龙年", @"辛己蛇年", @"壬午马年",
                        @"癸未羊年", @"甲申猴年", @"乙酉鸡年", @"丙戌狗年", @"丁亥猪年",
                        
                        @"戊子鼠年", @"己丑牛年", @"庚寅虎年", @"辛卯兔年", @"壬辰龙年", @"癸巳蛇年", @"甲午马年",
                        @"乙未羊年", @"丙申猴年", @"丁酉鸡年", @"戊戌狗年", @"己亥猪年",
                        
                        @"庚子鼠年", @"辛丑牛年", @"壬寅虎年", @"癸卯兔年", @"甲辰龙年", @"乙巳蛇年",@"丙午马年",
                        @"丁未羊年", @"戊申猴年", @"己酉鸡年", @"庚戌狗年", @"辛亥猪年",
                        
                        @"壬子鼠年", @"癸丑牛年", @"甲寅虎年", @"乙卯兔年", @"丙辰龙年", @"丁巳蛇年", @"戊午马年",
                        @"己未羊年", @"庚申猴年", @"辛酉鸡年", @"壬戌狗年", @"癸亥猪年"];
...
//    NSLog(@"嘻嘻哈哈哈year = %ld month = %ld day = %ld",yearIndex,monthIndex,dayIndex);
...
+(NSString *)getTheTimeBucket {
    ...
    if ([currentDate compare:[TimeTools getCustomDateWithHour:0]] == NSOrderedDescending && [currentDate compare:[self getCustomDateWithHour:9]] == NSOrderedAscending){
        return @"早上好";
    }else if ([currentDate compare:[TimeTools getCustomDateWithHour:9]] == NSOrderedDescending && [currentDate compare:[self getCustomDateWithHour:11]] == NSOrderedAscending)
    {
        return @"上午好";
    }else if ([currentDate compare:[TimeTools getCustomDateWithHour:11]] == NSOrderedDescending && [currentDate compare:[TimeTools getCustomDateWithHour:13]] == NSOrderedAscending)
    {
        return @"中午好";
    }else if ([currentDate compare:[TimeTools getCustomDateWithHour:13]] == NSOrderedDescending && [currentDate compare:[TimeTools getCustomDateWithHour:18]] == NSOrderedAscending){
        return @"下午好";
    }else{
        return @"晚上好";
    }
...
//    NSDateComponents *currentComps = [[NSDateComponents alloc] init];
    ...
    NSDateComponents *currentComps = [currentCalendar components:unitFlags fromDate:destinationDateNow];
    //设置当前的时间点
...
+(NSString*)weekdayStringFromDate:(NSDate*)inputDate{
    ...
    NSString *tempWeekday =  [weekdays objectAtIndex:theComponents.weekday];
    
    if ([[NSCalendar currentCalendar] isDateInToday:inputDate]) {
        tempWeekday = [NSString stringWithFormat:@"今天"];
    }
    if ([[NSCalendar currentCalendar] isDateInYesterday:inputDate]) {
        tempWeekday = [NSString stringWithFormat:@"昨天"];
    }
    if ([[NSCalendar currentCalendar] isDateInTomorrow:inputDate]) {
        tempWeekday = [NSString stringWithFormat:@"明天"];
    }
    return tempWeekday;
}
...
//            int mins = time/60;
//            dateStr = [NSString stringWithFormat:@"%d分钟前发布",mins];
...
+(NSString *)formateDate:(NSString *)dateString {
    @try {
        ...
        if (time<=60) {  //1分钟以内的
            dateStr = @"刚刚发布";
        }else if(time<=60*30){  //一个小时以内的
            int mins = time/60;
            if (mins < 30) {
                dateStr = [NSString stringWithFormat:@"%d分钟前发布",mins];
            }else{
                dateStr = [NSString stringWithFormat:@"半小时之前发布"];
            }
        }else if(time<=60*60*24) {
            dateStr = [NSString stringWithFormat:@"半小时之前发布"];
        }
