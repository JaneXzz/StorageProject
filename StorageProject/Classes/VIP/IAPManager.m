...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    ...
    if (product.count == 0) {
        ...
        NSLog(@"产品products==%@",product);
        NSLog(@"产品id==%@",response.invalidProductIdentifiers);
        NSLog(@"产品数量==========%lu",(unsigned long)product.count);
        for(SKProduct *oneProduct in product){
            NSLog(@"SKProduct描述信息%@", [oneProduct description]);
            NSLog(@"产品标题 %@", oneProduct.localizedTitle);
            NSLog(@"产品描述信息: %@", oneProduct.localizedDescription);
            NSLog(@"价格: %@", oneProduct.price);
            NSLog(@"Product id: %@", oneProduct.productIdentifier);
            SKMutablePayment *MpayMent = [SKMutablePayment paymentWithProduct:oneProduct];
...
- (void)completeTransaction:(SKPaymentTransaction *)transaction {
    ...
    if ([self.delegate respondsToSelector:@selector(successfulPurchase:)]) {
        [self.delegate successfulPurchase:nil];
    }
